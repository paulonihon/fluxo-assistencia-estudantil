...
export function TaskNode({ data, selected }: NodeProps) {
  ...
  const noId = data.id as string
  return (
    <div
      className="flex min-h-[76px] w-[200px] cursor-pointer flex-col justify-center rounded-2xl bg-gradient-to-b from-white to-[#FBFCFB] py-3 pl-4 pr-3 text-left transition-shadow"
      style={{
        borderLeft: `4px solid ${visual.cor}`,
        boxShadow: selected
    ...
    >
      ...
      <div className="mb-1.5 flex items-center justify-between gap-2">
        ...
        <span className="opacity-80">
          <NodeIcon noId={noId} cor={visual.cor} />
        </span>
      </div>
      <div className="text-[15px] font-semibold leading-snug text-[#14201A]">{data.rotulo as string}</div>
    </div>
  )
}
