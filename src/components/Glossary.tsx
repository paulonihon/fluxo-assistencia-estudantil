...
export function Glossary() {
  ...
  return (
    ...
    >
      <div
        className="flex h-[85vh] md:h-[70vh] w-full md:w-[560px] flex-col rounded-t-2xl md:rounded-2xl bg-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
          <div className="text-[16px] font-semibold text-gray-900">Glossário</div>
          <button
            type="button"
        ...
        <div className="px-4 py-2">
          <input
            ...
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-[14px] outline-none focus:border-emerald-600"
            value={busca}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          {termos.length === 0 ? (
            ...
            termos.map((t) => (
              <div key={t.sigla} className="border-b border-gray-100 py-3">
                <div className="text-[14px] font-semibold text-gray-900">
                  {t.sigla}
                  {t.nome !== t.sigla ? <span className="ml-2 font-normal text-gray-500">{t.nome}</span> : null}
                </div>
                <div className="mt-1 text-[13px] leading-relaxed text-gray-700">{t.definicao}</div>
              </div>
            ))
          )}
        </div>
